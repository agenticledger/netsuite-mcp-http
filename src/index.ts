#!/usr/bin/env node
/**
 * NetSuite MCP Server — Exposed via Streamable HTTP
 *
 * Auth model: Bearer token passthrough.
 * The client sends a Base64-encoded JSON object as the Bearer token containing
 ...
 * }
 *
 * The server extracts these and creates a per-session NetSuite API client.
...
app.get('/', (_req, res) => {
  ...
  res.json({
    ...
    version: '1.0.0',
    description:
      'Access Oracle NetSuite ERP records, SuiteQL queries, transactions, and accounting through MCP tools.',
    mcpEndpoint: '/mcp',
    ...
    auth: {
      type: 'dual-mode',
      description:
        'Pass a Base64-encoded JSON object with your NetSuite OAuth 1.0 credentials as the Bearer token. No credentials are stored on this server.',
      header: 'Authorization: Bearer <base64-encoded-credentials-json>',
      credentialsFormat: {
        accountId: 'Your NetSuite account ID (e.g. TSTDRV1234567)',
        consumerKey: 'OAuth 1.0 consumer key',
        consumerSecret: 'OAuth 1.0 consumer secret',
        tokenId: 'Token-based auth token ID',
        tokenSecret: 'Token-based auth token secret',
      },
      howToGetCredentials:
        'In NetSuite: Setup > Integration > Manage Integrations > New. Then Setup > Users/Roles > Access Tokens > New.',
    },
    configTemplate: {
      mcpServers: {
        netsuite: {
          ...
          headers: {
            Authorization: 'Bearer <base64-encoded-credentials-json>',
          },
        },
      },
    },
    links: {
      health: '/health',
      documentation: 'https://financemcps.agenticledger.ai/netsuite/',
    },
  });
});
...
<head>
  ...
  <title>NetSuite MCP Server — AgenticLedger</title>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    label{display:block;font-weight:500;margin-bottom:0.4rem;margin-top:0.6rem}
    input[type="text"],input[type="password"]{width:100%;padding:0.6rem 0.75rem;font-size:0.95rem;font-family:inherit;border:1px solid #cbd5e1;border-radius:8px;outline:none;transition:border-color .15s}
...
<body>
  ...
  <div class="container">
    <h1>NetSuite MCP Server</h1>
    <p class="subtitle">Access Oracle NetSuite ERP records, SuiteQL queries, transactions, and accounting through ${tools.length} MCP tools.</p>

    <div class="card">
      <h2>How to get your NetSuite credentials</h2>
      <ol class="steps">
        <li>In NetSuite, go to <strong>Setup &gt; Integration &gt; Manage Integrations &gt; New</strong>.</li>
        <li>Enable <strong>Token-Based Authentication</strong> and note the <strong>Consumer Key</strong> and <strong>Consumer Secret</strong>.</li>
        <li>Go to <strong>Setup &gt; Users/Roles &gt; Access Tokens &gt; New</strong>.</li>
        <li>Select the integration, user, and role. Note the <strong>Token ID</strong> and <strong>Token Secret</strong>.</li>
        <li>Your <strong>Account ID</strong> is in <strong>Setup &gt; Company &gt; Company Information</strong>.</li>
        <li>Fill in the fields below to generate your config.</li>
      </ol>
    </div>
    ...
    <div class="card">
      ...
      <input type="text" id="accountId" placeholder="e.g. TSTDRV1234567" autocomplete="off" spellcheck="false" />
      <label for="consumerKey">Consumer Key</label>
      <input type="password" id="consumerKey" placeholder="OAuth 1.0 consumer key" autocomplete="off" />
      <label for="consumerSecret">Consumer Secret</label>
      <input type="password" id="consumerSecret" placeholder="OAuth 1.0 consumer secret" autocomplete="off" />
      <label for="tokenId">Token ID</label>
      <input type="password" id="tokenId" placeholder="Token-based auth token ID" autocomplete="off" />
      <label for="tokenSecret">Token Secret</label>
      <input type="password" id="tokenSecret" placeholder="Token-based auth token secret" autocomplete="off" />
  ...
  <script>
    const fields = ['accountId', 'consumerKey', 'consumerSecret', 'tokenId', 'tokenSecret'];
    ...
    function buildConfig() {
      ...
      let token = 'YOUR_BASE64_TOKEN';
      if (anyFilled) {
        token = btoa(JSON.stringify(vals));
      }
...
function extractCredentials(req: express.Request): NetSuiteCredentials | null {
  ...
  try {
    ...
    if (
      ...
      return creds as NetSuiteCredentials;
    }
    return null;
  } catch {
    return null;
...
function createMCPServer(client: NetSuiteClient): Server {
...
app.post('/mcp', async (req, res) => {
  ...
  // New session — requires Bearer token with NetSuite credentials
  const creds = extractCredentials(req);
  if (!creds) {
    res.status(401).json({
      error:
        'Missing or invalid Authorization header. Use: Bearer <base64-encoded-json> where JSON contains: accountId, consumerKey, consumerSecret, tokenId, tokenSecret',
    });
    return;
  }
...
<head>
  ...
  <title>Authorize \u2014 NetSuite MCP</title>
...
<body>
  <div class="card">
    <div class="header"><img src="/static/logo.png" alt="AgenticLedger"><span>NetSuite MCP</span></div>
    <div class="consent-msg">An application wants to connect to <strong>NetSuite MCP Server</strong> on your behalf. Enter your API key to authorize access.</div>
...
app.listen(PORT, () => {
  console.log(`NetSuite MCP HTTP Server running on port ${PORT}`);
